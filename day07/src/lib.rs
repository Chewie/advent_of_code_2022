...
impl FileSystem {
    ...
    pub fn total_size_under_100k(&self) -> usize {
        let mut good_dirs = Vec::new();
        self.root.borrow().all_dirs_size(&mut good_dirs);
        good_dirs.iter().sum()
    }
...
impl DirEntry {
    fn dump(&self, writer: &mut impl std::fmt::Write, dir_name: &str, indent_size: usize) {
        ...
        write!(writer, "{indent}- {dir_name} (dir)\n").unwrap();
        for (name, size) in &self.files {
            write!(writer, "{indent}  - {name} (file, size={size})\n").unwrap();
        }
    ...
    fn all_dirs_size(&self, mut good_dirs: &mut Vec<usize>) -> usize {
        let files_size: usize = self.files.values().sum();
        let dirs_size: usize = self
            .subdirs
            .values()
            .map(|subdir| subdir.borrow().all_dirs_size(&mut good_dirs))
            .sum();
